...
pub fn integral_image(base: &GrayImage) -> Vec<Vec<f32>> {
    ...
    let mut value_holder: Vec<Vec<f32>> = vec![vec![0.0;(base_cols+1) as usize];(base_rows+1) as usize];
...
pub fn image_raised_power(base: &GrayImage, power: f32 ) -> Vec<Vec<f32>>{ 
    ...
    for row in 0..base_rows {
        for col in 0..base_cols{
            float_result[row as usize][col as usize] = (*base.get_pixel(col, row).channels().get(0).unwrap() as f32).powf(power);
        }
    }
...
pub fn local_statistics(base: &GrayImage, window_height: u32, window_width: u32) -> (GrayImage, GrayImage) {
    ...
    image::imageops::overlay(&mut zero_pad_base, base, ((window_width+1)/2) as i64, ((window_height+1)/2) as i64);
    ...
    let mut mean_value_holder: Vec<Vec<f32>> = vec![vec![0.0;base_cols as usize];base_rows as usize];
    let mut standard_dev_value_holder: Vec<Vec<f32>> = vec![vec![0.0;base_cols as usize];base_rows as usize];
    ...
    for row in 0..base_rows {
        for col in 0..base_cols {
            ...
            squared_window_sum = integral_squared[ (row + window_height) as usize][ (col+window_width) as usize]
    ...
    mean_value_holder.iter_mut()
    .flat_map(|row| row.iter_mut())
    .for_each(|item| *item = *item/mean_max*255.0);

    standard_dev_value_holder.iter_mut()
    .flat_map(|row| row.iter_mut())
    .for_each(|item| *item = *item/standard_dev_max*255.0);

    for row in 0..base_rows {
        for col in 0..base_cols {
            let mean_pixel: image::Luma::<u8> = image::Luma::<u8>([mean_value_holder[row as usize][col as usize] as u8]);
            let standard_dev_pixel: image::Luma::<u8> = image::Luma::<u8>([standard_dev_value_holder[row as usize][col as usize] as u8]);
